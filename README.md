...
# If you already have this project locally (this workspace), you can skip cloning.
```
...
## application.properties (project example)

Below is an example of the `application.properties` used by this project. Copy this into `src/main/resources/application.properties` and update values (especially the database password and any secrets) before running the application.

```properties
spring.application.name=ECommerce

# Spring Security - Default User Credentials (optional)
#spring.security.user.name=ecommerce_user
#spring.security.user.password=EcommercePass@2024
...
Notes:
- Replace `<your_password>` with your MySQL root (or configured user) password.
- The example uses database `ecommerce_data`. Create it or change the URL to the DB you created (see "Configure Database" section).
- For production, do NOT commit real credentials to the repository; use environment variables or an external config store.
